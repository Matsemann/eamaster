...
import com.matsemann.simulation.SimulationViewer;
...
public class ActionFactory implements ControllerListener {
	...
	protected void initialize() {
        ...
        tasksAction = new AbstractAction() {

            private static final long serialVersionUID = 377019999031491835L;

            {
                putValue(Action.NAME, localization.getString("action.tasks.name"));
                putValue(Action.SHORT_DESCRIPTION, localization.getString("action.clear.description"));
                setEnabled(!controller.isRunning());
            }
	...
	public void controllerStateChanged(ControllerEvent event) {
		if (event.getType().equals(ControllerEvent.Type.STATE_CHANGED)) {
			...
			getShowStatisticsAction().setEnabled(true);
//			getShowStatisticsAction().setEnabled((problems.size() == 1) && 	(algorithms.size() > 1));
    ...
    // Mats stuff
    public Action getShowBulletWindowAction(final ResultKey key) {
        return new AbstractAction() {

            private static final long serialVersionUID = 168052984883777744L;

            {
                putValue(Action.NAME, localization.getString("action.showBulletWindow.name"));
                putValue(Action.SHORT_DESCRIPTION, localization.getString("action.showBulletWindow.description"));
            }

            @Override
            public void actionPerformed(ActionEvent e) {
                List<Accumulator> accumulators = controller.get(key);
                ...
                for (Accumulator accumulator : accumulators) {
                    ...
                    List<?> list = (List<?>)accumulator.get("Approximation Set", accumulator.size("Approximation Set")-1); // get the last solutions
                ...
                new SimulationViewer(population, referenceSet);
                // show window

//                new ReferenceManager(key.getProblem()).mergeSolutions(population);
//
//
//                ApproximationSetViewer viewer = new ApproximationSetViewer(
//                        key.toString(),
...
//                viewer.setVisible(true);
    ...
    public Action getAddToReferenceSetAction(final ResultKey key) {
        return new AbstractAction() {

            private static final long serialVersionUID = 1680529999777744L;

            {
                putValue(Action.NAME, localization.getString("action.addToReference.name"));
                putValue(Action.SHORT_DESCRIPTION, localization.getString("action.addToReference.description"));
            }
