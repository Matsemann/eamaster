...
import com.matsemann.ea.TaskManager;
...
ControllerListener {
	...
	protected void initialize() {
		...
		resultTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(final MouseEvent e) {
				if (SwingUtilities.isRightMouseButton(e)) {
                    ...
                    popupMenu.add(new JMenuItem(
                            actionFactory.getAddToReferenceSetAction(
                                    resultListModel.getElementAt(index))));
                    popupMenu.add(new JMenuItem(
                            actionFactory.getShowBulletWindowAction(
                                    resultListModel.getElementAt(index))));
        ...
        runTasks = new JButton(actionFactory.getTasksAction());
...
//            numberOfSeeds.se
    ...
    public void runNextTask() {
        TaskManager.RunTask nextTask = TaskManager.getInstance().getNextTask();
        if (nextTask != null) {
            algorithm.setSelectedItem(nextTask.algorithm);
            problem.setSelectedItem(nextTask.problem);
            numberOfSeeds.setValue(nextTask.seeds);
            numberOfEvaluations.setValue(nextTask.nfe);

            controller.properties.clear();

            for (Map.Entry<String, String> es : nextTask.properties.entrySet()) {
                controller.properties.setProperty(es.getKey(), es.getValue());
            }
            ...
            ResultKey oldKey = new ResultKey("NSGA-IIId", "wheel-2obj-free");
            ResultKey newKey = new ResultKey("NSGA-IIId-2obj-really", "wheel-4obj-free");
            List<Accumulator> accumulators = controller.get(oldKey);
            if (accumulators != null) {
                for (Accumulator accumulator : accumulators) {
                    NondominatedPopulation population = new NondominatedPopulation();
                    ArrayList<Solution> solutions = new ArrayList<>();

                    List<?> appSet = (List<?>) accumulator.get("Approximation Set", accumulator.size("Approximation Set") - 1);

                    for (Object o : appSet) {
                        Solution s = (Solution) o;
                        Solution newSolution = new Solution(0, 4);

                        Task task = (Task) s.getAttribute("task");

                        newSolution.setAttribute("task", task);
                        RepFull rep = new RepFull(task.result);

                        newSolution.setObjective(0, rep.dst);
                        newSolution.setObjective(1, rep.rotZ);
                        newSolution.setObjective(2, rep.rot);
                        newSolution.setObjective(3, rep.spokes);
                        population.add(newSolution);
                        solutions.add(newSolution);
                    }

                    Accumulator newAccumulator = new Accumulator();
                    newAccumulator.add("Approximation Set", solutions);

                    controller.add(newKey, newAccumulator);
                }


            }
//            try {
//                controller.saveData(new File("plzsave2.dat"));
//            } catch (IOException e) {
//                e.printStackTrace();
//            }
        }
